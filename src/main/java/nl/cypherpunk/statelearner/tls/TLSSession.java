...
import nl.cypherpunk.statelearner.tls.messages.Alert;
import nl.cypherpunk.statelearner.tls.messages.Certificate;
import nl.cypherpunk.statelearner.tls.messages.CertificateRequest;
import nl.cypherpunk.statelearner.tls.messages.CertificateVerify;
import nl.cypherpunk.statelearner.tls.messages.ClientHello;
import nl.cypherpunk.statelearner.tls.messages.ClientKeyExchange;
import nl.cypherpunk.statelearner.tls.messages.Finished;
import nl.cypherpunk.statelearner.tls.messages.HandshakeMsg;
import nl.cypherpunk.statelearner.tls.messages.ServerHello;
import nl.cypherpunk.statelearner.tls.messages.ServerHelloDone;
import nl.cypherpunk.statelearner.tls.messages.ServerKeyExchange;
...
public class TLSSession {
	...
	public TLSSession() throws Exception {
	...
	public String receiveMessages(InputStream input) throws Exception {
		return receiveMessages(input, null);
	}
	
	public String receiveMessages(InputStream input, List<byte[]> applicationContent) throws Exception {
		String out = "";
		...
		while (input.available() > 0) {
			...
			while(payloadStream.available() > 0) {
				...
				case TLS.CONTENT_TYPE_APPLICATION:
					...
					if(applicationContent != null) {
						byte[] appMsg = new byte[record.getLength()];
						payloadStream.read(appMsg);
						applicationContent.add(appMsg);
					} else {
			...
			if(contentType == -1 && !(input instanceof ByteArrayInputStream)) {
				// We got to the end of the stream
	...
	public byte[] buildMessage(byte type, byte[] msg) throws Exception {
	...
	public byte[] buildHandshakeMessage(HandshakeMsg msg, boolean updateHash) throws Exception {
		if(updateHash) {
		...
		return buildMessage(TLS.CONTENT_TYPE_HANDSHAKE, msg.getBytes());
	}

	public byte[] buildHandshakeMessage(HandshakeMsg msg) throws Exception {
		return buildHandshakeMessage(msg, true);
	}

	public byte[] buildHeartbeatRequest() throws Exception {
		byte[] msg = new byte[] {
		...
		return buildMessage(TLS.CONTENT_TYPE_HEARTBEAT, msg);
	}

	public byte[] buildHeartbeatResponse() throws Exception {
		byte[] msg = new byte[] {
	...
	public byte[] buildServerHelloRSA() throws Exception {
		...
		return buildHandshakeMessage(sh);
	}

	public byte[] buildServerHelloDHE() throws Exception {
		// Generate server_random
		rand.nextBytes(server_random);
	...
	public byte[] buildClientHello(byte[] ciphersuites, boolean buffer_reset) throws Exception {
		...
		return buildHandshakeMessage(ch);
	}

	public byte[] buildClientHelloAll() throws Exception {
		//return buildClientHello(Utils.concat(Utils.concat(CipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA, CipherSuite.TLS_RSA_WITH_3DES_EDE_CBC_SHA), Utils.concat(CipherSuite.TLS_DHE_RSA_WITH_AES_128_CBC_SHA, Utils.concat(CipherSuite.TLS_DHE_RSA_WITH_3DES_EDE_CBC_SHA, Utils.concat(CipherSuite.TLS_DHE_RSA_WITH_AES_256_GCM_SHA384, CipherSuite.TLS_DHE_RSA_WITH_AES_256_CBC_SHA)))), false);
		return buildClientHello(Utils.concat(Utils.concat(CipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA, CipherSuite.TLS_RSA_WITH_3DES_EDE_CBC_SHA), Utils.concat(CipherSuite.TLS_DHE_RSA_WITH_AES_128_CBC_SHA, Utils.concat(CipherSuite.TLS_DHE_RSA_WITH_3DES_EDE_CBC_SHA, CipherSuite.TLS_DHE_RSA_WITH_AES_256_CBC_SHA))), false);
		//return buildClientHello(CipherSuite.TLS_DHE_RSA_WITH_AES_256_CBC_SHA, false);
	}

	public byte[] buildClientHelloRSA() throws Exception {
		return buildClientHello(CipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA, false);
		//return buildClientHello(Utils.concat(CipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA, CipherSuite.TLS_RSA_WITH_3DES_EDE_CBC_SHA));
	}

	public byte[] buildClientHelloRSAReset() throws Exception {
		return buildClientHello(CipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA, true);
		//return buildClientHello(Utils.concat(CipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA, CipherSuite.TLS_RSA_WITH_3DES_EDE_CBC_SHA));
	}

	public byte[] buildClientHelloDHE() throws Exception {
		return buildClientHello(Utils.concat(CipherSuite.TLS_DHE_RSA_WITH_AES_128_CBC_SHA, CipherSuite.TLS_DHE_RSA_WITH_3DES_EDE_CBC_SHA), false);
	}

	public byte[] buildClientHelloDHEReset() throws Exception {
		return buildClientHello(Utils.concat(CipherSuite.TLS_DHE_RSA_WITH_AES_128_CBC_SHA, CipherSuite.TLS_DHE_RSA_WITH_3DES_EDE_CBC_SHA), true);
	}

	public byte[] buildClientHello2() throws Exception {
		// Generate client_random
		rand.nextBytes(client_random);
	...
	public byte[] buildEmptyCertificate() throws Exception {
		...
		return buildHandshakeMessage(cm);
	}
	...
	public byte[] buildServerCertificate() throws Exception {
		Certificate cm = new Certificate(new X509Certificate[] { serverCertificate });
		return buildHandshakeMessage(cm);
	}

	public byte[] buildCertificateRequest() throws Exception {
		...
		return buildHandshakeMessage(new CertificateRequest(cert_types, supported_algorithms, distinguished_names));
	}

	public byte[] buildClientCertificateVerify() throws IOException, Exception {
		//byte[] signature = Crypto.SIGN_RSA_SHA384(clientPrivateKey, handshakeMessages);
		//return buildHandshakeMessage(new CertificateVerify(Crypto.HASH_SIGNATURE_ALGORITHM_SHA384RSA, signature));		
		byte[] signature = Crypto.SIGN_RSA_SHA256(clientPrivateKey, handshakeMessages);
		return buildHandshakeMessage(new CertificateVerify(Crypto.HASH_SIGNATURE_ALGORITHM_SHA256RSA, signature));		
		//byte[] signature = Crypto.SIGN_RSA_SHA1(clientPrivateKey, handshakeMessages);
		//return buildHandshakeMessage(new CertificateVerify(Crypto.HASH_SIGNATURE_ALGORITHM_SHA1RSA, signature));		
			
	}

	public byte[] buildServerKeyExchange() throws Exception {
		...
		return buildHandshakeMessage(ske);
	}

	public byte[] buildServerHelloDone() throws Exception {
		ServerHelloDone shd = new ServerHelloDone();
		return buildHandshakeMessage(shd);
	}
	...
	public byte[] buildClientKeyExchange() throws Exception {
		...
		case CipherSuite.ALG_DHE_RSA:
			...
			dh_Ys = Arrays.copyOfRange(dh_Ys, i, dh_Ys.length);
			cke = new ClientKeyExchange(dh_Ys);
			break;
		...
		return buildHandshakeMessage(cke);
	}

	public byte[] buildChangeCipherSpec() throws Exception {
		byte[] msg = buildMessage(TLS.CONTENT_TYPE_CCS, new byte[] {0x01});
	...
	public byte[] buildChangeCipherSpec2() throws Exception {
		byte[] msg = buildMessage(TLS.CONTENT_TYPE_CCS, new byte[] {0x01});
		//master_secret = new byte[] {};
		//Arrays.fill(client_random, (byte) 0x00);
		//Arrays.fill(server_random, (byte) 0x00);
	...
	public byte[] buildChangeCipherSpec3() throws Exception {
		byte[] msg = buildMessage(TLS.CONTENT_TYPE_CCS, new byte[] {0x01});
		//master_secret = new byte[] {};
		//Arrays.fill(client_random, (byte) 0x00);
		//Arrays.fill(server_random, (byte) 0x00);
	...
	public byte[] buildAlert10() throws Exception {
		Alert alert = new Alert((byte)1, (byte)0);

		return buildMessage(TLS.CONTENT_TYPE_ALERT, alert.getBytes());
	}

	public byte[] buildAlert1100() throws Exception {
		Alert alert = new Alert((byte)1, (byte)100);

		return buildMessage(TLS.CONTENT_TYPE_ALERT, alert.getBytes());
	}

	public byte[] buildAlert1255() throws Exception {
	...
	public byte[] buildFinished() throws Exception {
		...
		return buildHandshakeMessage(finished);
	}
	...
	public byte[] buildApplicationData() throws Exception {
		...
		return buildMessage(TLS.CONTENT_TYPE_APPLICATION, req.getBytes());
	}
	
	public byte[] buildApplicationData(byte[] payload) throws Exception {
		return buildMessage(TLS.CONTENT_TYPE_APPLICATION, payload);
	}

	public byte[] buildApplicationDataEmpty() throws Exception {
		return buildMessage(TLS.CONTENT_TYPE_APPLICATION, new byte[] {});
	}
}
